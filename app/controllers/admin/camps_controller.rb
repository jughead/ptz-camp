...
module Admin
  class CampsController < Admin::ApplicationController
    ...
    def download_teams
      temp_file = Camps::GenerateTeamsCsvCommand.new(@camp).execute
      send_file temp_file.path, type: 'text/csv', disposition: 'attachment', filename: 'teams.csv'
    end

    def download_participants
      temp_file = Camps::GenerateParticipantsCsvCommand.new(@camp, params[:columns]&.split(',')&.map(&:strip)).execute
